...
<head>
    ...
    <title>Jurnal Memorial - {{ $jurnal->bukti }}</title>
    <style>
        body {
        ...
        .total-value { font-size: 15px; font-weight: 700; color: #6c757d; }
        ...
        .section-title { font-size: 13px; font-weight: 700; margin: 25px 0 10px 0; padding: 5px 10px; background-color: #e9ecef; color: #6c757d; border-left: 4px solid #6c757d; }
        table { width: 100%; border-collapse: collapse; margin: 15px 0; }
        th, td { border: 1px solid #dee2e6; padding: 8px 10px; text-align: left; vertical-align: top; font-size: 11px; }
        th { background-color: #6c757d; color: white; font-weight: 600; text-align: center; }
        .text-center { text-align: center; }
        .text-right { text-align: right; }
        .amount { font-family: 'Consolas', 'Courier New', monospace; font-weight: 700; color: #333; font-size: 12px; }
        ...
        .total-row { background-color: #6c757d !important; color: white; font-weight: 700; font-size: 12px; }
...
<body>
    <div class="container">
        ...
        <div class="document-title">JURNAL MEMORIAL</div>

        <div class="jurnal-info-wrapper">
            <table class="info-table">
                <tr>
                    <td width="50%">
                        <div><span class="info-label">NO. BUKTI :</span><span class="info-value"><strong>{{ $jurnal->bukti }}</strong></span></div>
                        <div><span class="info-label">NO. REFERENSI :</span><span class="info-value">{{ $jurnal->no_reff }}</span></div>
        ...
        <div class="section-title">DETAIL ENTRI JURNAL</div>
        <table>
            <thead>
                <tr>
                    <th width="5%">No</th>
                ...
                @if($jurnal->details && $jurnal->details->count() > 0)
                    @foreach($jurnal->details as $index => $detail)
                        <tr>
                            ...
                            <td class="text-right amount">{{ $detail->debit > 0 ? number_format($detail->debit, 0, ',', '.') : '-' }}</td>
                            <td class="text-right amount">{{ $detail->credit > 0 ? number_format($detail->credit, 0, ',', '.') : '-' }}</td>
                        </tr>
