...
<head>
    ...
    <title>Jurnal Pemakaian Bahan - {{ $jurnal->no_reff }}</title>
    <style>
        body {
        ...
        .total-value { font-size: 15px; font-weight: 700; color: #fd7e14; }
        ...
        .section-title { font-size: 13px; font-weight: 700; margin: 25px 0 10px 0; padding: 5px 10px; background-color: #e9ecef; color: #fd7e14; border-left: 4px solid #fd7e14; }
        table { width: 100%; border-collapse: collapse; margin: 15px 0; }
        th, td { border: 1px solid #dee2e6; padding: 8px 10px; text-align: left; vertical-align: top; font-size: 11px; }
        th { background-color: #fd7e14; color: white; font-weight: 600; text-align: center; }
        .text-center { text-align: center; }
        .text-right { text-align: right; }
        .amount { font-family: 'Consolas', 'Courier New', monospace; font-weight: 700; color: #fd7e14; font-size: 12px; }
        ...
        .total-row { background-color: #fd7e14 !important; color: white; font-weight: 700; font-size: 12px; }
...
<body>
    <div class="container">
        ...
        <div class="document-title">JURNAL PEMAKAIAN BAHAN</div>
            ...
            <table class="info-table">
                <tr>
                    <td width="50%">
                        ...
                        <div><span class="info-label">NO. BUKTI :</span><span class="info-value">{{ $jurnal->no_bukti }}</span></div>
                        <div><span class="info-label">TANGGAL :</span><span class="info-value">{{ $jurnal->tanggal->format('d M Y') }}</span></div>
                    </td>
                    <td width="50%">
                        <div><span class="info-label">TOTAL PEMAKAIAN:</span><span class="info-value total-value">Rp {{ number_format($jurnal->rp, 0, ',', '.') }}</span></div>
                        ...
                        @if($jurnal->kodeProyek)
                            <div><span class="info-label">KODE PROYEK:</span><span class="info-value">{{ $jurnal->kodeProyek->name }}</span></div>
                        @endif
                    </td>
                </tr>
            </table>
        ...
        <div class="section-title">DETAIL PEMAKAIAN BAHAN</div>
        <table>
            <thead>
                <tr>
                    <th width="5%">No</th>
                ...
                @if($jurnal->details && $jurnal->details->count() > 0)
                    @foreach($jurnal->details as $index => $detail)
                        <tr>
                            ...
                            <td class="text-right amount">Rp {{ number_format($detail->jumlah, 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                @else
                    <tr>
            ...
            <tfoot>
                <tr class="total-row">
                    <td colspan="3" class="text-right">TOTAL PEMAKAIAN DEBIT</td>
