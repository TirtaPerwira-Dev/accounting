...
use App\Filament\Accounting\Resources\NomorBantuResource\Pages;
use App\Filament\Accounting\Resources\NomorBantuResource\RelationManagers;
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('rekening.kelompok.no_kel')
                    ->label('No. Kelompok')
                    ->searchable()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
                ...
                TextColumn::make('nomor_akuntansi')
                    ...
                    ->searchable(query: function (Builder $query, string $search): Builder {
                        ...
                        return $query->whereHas('rekening.kelompok', function (Builder $query) use ($search) {
                            ...
                            $query->whereRaw(
                                "CONCAT(
                                    LPAD(kelompoks.no_kel, 2, '0'),
                                    LPAD((SELECT no_rek FROM rekenings WHERE id = nomor_bantus.rekening_id), 4, '0'),
                                    LPAD(nomor_bantus.no_bantu, 2, '0')
                                ) LIKE ?",
                                ['%' . $search . '%']
                            );
                        })->orWhere(function (Builder $query) use ($search) {
                            // Also search in individual parts
                            $query->where('nomor_bantus.no_bantu', 'LIKE', '%' . $search . '%')
                                ->orWhereHas('rekening', function (Builder $subQuery) use ($search) {
                                    $subQuery->where('no_rek', 'LIKE', '%' . $search . '%')
                                        ->orWhereHas('kelompok', function (Builder $kelompokQuery) use ($search) {
                                            $kelompokQuery->where('no_kel', 'LIKE', '%' . $search . '%');
                                        });
                                });
                        });
                    })
